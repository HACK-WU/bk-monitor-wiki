...
## 契约层就绪
C0 + C1 + C2 + C5 就绪（C5 于 2026-08-31 补建，含 9 条有证据决策 + 2 条 `[推测]`）
...
- **契约层**（根目录，黑盒使用文档）：
  ...
  - `C5-关键决策.md` — 11 条关键决策（内置视图读时同步 / 默认配置深拷贝 / 内置视图缓存 / 进程 display_name 分组 / 端口健康数据源 / 指标单位与精度 / 时点快照并发 / panel_count 分级 / APM 忽略 type 等）
