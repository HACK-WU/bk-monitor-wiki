Monitor API 资源自动暴露到 OpenAI 的机制：monitor_web/monitor_api/views/ 下新增的 ResourceViewSet 会自动生成 OpenAI entry point，可通过 resource.xxx.yyy() 调用。
- 位置: `monitor_web/monitor_api/views/`
- 自动注册机制：每个新类默认自动生成一个 OpenAI entry point / `resource` 路径入口
