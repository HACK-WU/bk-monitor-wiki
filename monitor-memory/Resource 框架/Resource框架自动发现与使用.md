Resource 框架自动发现与使用方式：ResourceManager 自动扫描各模块的 resource/adapter/api 目录，提供全局入口和显式导入两种使用方式。
- 位置: `core/drf_resource/management/root.py`
- 自动发现入口：`ResourceManager` 自动扫描各模块的 resource/adapter/api 目录
