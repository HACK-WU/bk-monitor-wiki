IssueQueryHandler ES 查询构建逻辑，包含字段映射表 QUERY_FIELD_MAP、QSearch 关键词模式、排序规则与合并查询等特殊逻辑。

- 符号: `IssueQueryHandler`（继承 `BaseBizQueryHandler[IssueDocument]`）
- 位置: `packages/fta_web/issue/handlers/issue.py`
- 查询条件来源: `IssueSearchSerializer` → `BaseSearchSerializer`
