IssueViewSet 权限控制模式，通过 READ_ONLY_ENDPOINTS / NO_BIZ_REQUIRED_ENDPOINTS 区分只读与业务校验端点，并定义 IssueBusinessActionPermission 处理批量操作的 IAM 权限。

- 符号: `IssueViewSet`、`READ_ONLY_ENDPOINTS`、`NO_BIZ_REQUIRED_ENDPOINTS`、`IssueBusinessActionPermission`
- 位置: `packages/fta_web/issue/views.py`
- 路由注册: `register(r"", IssueViewSet, basename="issue")`
