SceneViewModel 是监控平台「场景视图（监控大盘）」的持久化配置单元，对应数据库表，唯一约束 `unique_together = (bk_biz_id, scene_id, type, id)`。

- 符号: `SceneViewModel`
- 位置: `bkmonitor/packages/monitor_web/models/scene_view.py`
...
### 内置场景默认视图由 Processor 灌入此表

内置场景默认视图由 BuiltinProcessor 读取内置 JSON 配置，bulk_create 成 SceneViewModel（首次访问惰性创建，`create_default_views`）。

- 符号: `BUILTIN_SCENES`、`BuiltinProcessor`、`create_default_views`、`get_view_config`
...
### scene_view 系列 Resource 提供 CRUD/查询

scene_view 系列 Resource 提供场景视图的 CRUD 与查询能力。

- 符号: `GetSceneViewListResource`、`GetSceneViewResource`、`UpdateSceneViewResource`、`DeleteSceneViewResource`、`BulkUpdateSceneViewOrderAndNameResource`、`GetSceneViewDimensionsResource`、`GetSceneViewDimensionValueResource`
...
### 模型自带 update_order 方法

SceneViewModel 自带 update_order 方法，用 `atomic` + `select_for_update`，把当前视图插到 `SceneViewOrderModel.config` 指定位置，实现拖拽排序。

- 符号: `SceneViewModel.update_order()`
- 位置: `bkmonitor/packages/monitor_web/models/scene_view.py`

### UpdateSceneViewResource 更新/落库逻辑

名义"更新场景视图配置"，实际既能更新也能创建（因 `id` 可选），两段式执行。

- 符号: `UpdateSceneViewResource`、`create_or_update_view`
- 位置: `bkmonitor/packages/monitor_web/scene_view/resources/view.py`

两段式执行：

1. 排序表更新：仅当请求带非空 `view_order` 时，`SceneViewOrderModel.objects.get_or_create` 后整体覆盖 `config` 为视图ID顺序数组（全量覆盖，非 `update_order` 的增量挪位）
2. 确定视图ID：有 `id` 则更新已有视图，无则生成 `custom_{SceneViewModel.objects.count()}` 走新建
3. 落库 SceneViewModel：调用 `create_or_update_view`（builtin/__init__.py）按 `scene_id` 路由到对应 BuiltinProcessor
...
- 排序写入两套路径并存：`UpdateSceneViewResource`(全量数组覆盖) vs `SceneViewModel.update_order()`(scene_view.py 单视图增量挪位)，不可混用
...
### GetSceneViewListResource 查询/排序/字段映射

列出某业务 + 场景 + 类型(overview/detail)下的所有视图元信息，按 `SceneViewOrderModel` 排序返回，不渲染图表。

- 符号: `GetSceneViewListResource`、`get_view_list`、`perform_request`、`get_panel_count`
- 位置: `bkmonitor/packages/monitor_web/scene_view/resources/view.py`

执行流程（get_view_list）：

1. 查库：`SceneViewModel.objects.filter(bk_biz_id, scene_id, type)`
2. 惰性补齐内置视图：`create_default_views(...)` 首次访问灌入，再重查
3. 组装列表：非 kubernetes 优先用 `list_processors_view`(processor 自定义列表)否则 DB views，逐视图 `get_view_config(only_simple_info=True)`，跳过 `hidden`；kubernetes 单独分支只取 mode，不计算 panel_count，show_panel_count 硬编码 False
4. 排序：查 `SceneViewOrderModel.config`，按视图ID顺序排 result(不在顺序里的排最后)；processor 自定义排序走 `sort_view_list`

perform_request：scene_id != kubernetes 且不传 type 时，自动拼接 overview + detail 两列表返回；传 type 只返回该类型
...
- panel_count：`get_panel_count(view_config)` 递归统计 panels，排除 row 容器和 tag-chart

视图类型两层含义：① `type` 维度 overview/detail 两层；② 同类型下按 id 区分(host/process/cluster 等)

### GetSceneResource 作用 + SceneModel 写入点

GetSceneResource 返回场景分类列表，结果 = 内置场景(BUILTIN_SCENES 常量, builtin=True) + 用户自定义场景(SceneModel 表, builtin=False) 拼接；前端据此渲染左侧场景导航。

- 符号: `GetSceneResource`、`BUILTIN_SCENES`、`SceneModel`
- 位置: `bkmonitor/packages/monitor_web/scene_view/resources/view.py`

- 内置场景不入表，直接来自 `builtin/__init__.py` 的 `BUILTIN_SCENES` 字典；用户场景来自 SceneModel 查询
- SceneModel 模型定义位置：`monitor_web/models/scene_view.py`，字段 unique_id/bk_biz_id/id/name/data_range/view_order，唯一约束 (bk_biz_id, id)
...
2. DeleteSceneViewResource 删视图时更新 `scene.view_order` 后 save ——更新已有记录，非新建
...
内置 JSON 配置随代码包发布，首次访问场景时由 create_default_views 把 JSON bulk_create 进 SceneViewModel 表（JSON 是源、库表是运行时副本）。

- 位置: `bkmonitor/packages/monitor_web/scene_view/builtin/view_configs/`（仓库根下）
- 共 44 个 json 文件，跟随代码仓库提交与版本发布；不进数据库、不随业务运行态变化（内置视图的"源数据"）
- 运行时库表镜像：首次访问某场景时，`create_default_views` 把这些 JSON `bulk_create` 进 `SceneViewModel` 表；真正被接口读取的是库表记录，而非这个目录文件

加载机制：

- 符号: `BuiltinProcessor._read_builtin_view_config`、`NormalProcessorMixin.load_builtin_views`
- 位置: `bkmonitor/packages/monitor_web/scene_view/builtin/__init__.py`
- `BuiltinProcessor._read_builtin_view_config` 读取 `{view_config_path}/{filename}.json`；`view_config_path` = builtin 目录下的 view_configs 子目录
- 各 `BuiltinProcessor` 通过 `filenames` 类属性声明加载哪些 JSON；`NormalProcessorMixin.load_builtin_views` 遍历 filenames 读入 `cls.builtin_views`
