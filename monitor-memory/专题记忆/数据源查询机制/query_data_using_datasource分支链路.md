...
relation: query_data_using_datasource分支链路
keywords: [use_unify_query, DataQueryHandler, query_data_using_datasource, _query_data_using_datasource, api.metadata.get_es_data]
exportedAt: "2026-07-14T03:33:42.383Z"
---
# `_query_data_using_datasource` 分支总结

> 对应入口：`bkmonitor/bkmonitor/data_source/unify_query/query.py`
> - 分支判定：`UnifyQuery.use_unify_query()` → `False`
> - 分支实现：`UnifyQuery._query_data_using_datasource()`（query.py:588）
> - 调用方：`UnifyQuery._query_data_internal()` 的 `else` 分支（query.py:736-750）
>
> 本文档聚焦 `_query_unify_query` 之外的另一条路径：当查询**不走统一查询模块（unify-query HTTP 接口）**时，由 `UnifyQuery` 直接把查询**下推到各数据源原生 `query_data`**，再由数据源自身构造原生存储查询（`DataQueryHandler`）。

## 1. 分支定位与触发条件

`UnifyQuery._query_data_internal` 在 `self.use_unify_query()` 返回 `False` 时进入本分支（query.py:716、736）。

`use_unify_query()`（query.py:342）返回 `False` 的全部情形：

| # | 条件 | 代码位置 | 说明 |
|---|------|----------|------|
| 1 | `data_sources[0].id` **不在** `UnifyQueryDataSources + GrayUnifyQueryDataSources` | query.py:347-348 | **恒走本分支** |
| 2 | 灰度数据源 `switch_unify_query(bk_biz_id)` 返回 `False` | query.py:364-368 | 仅 `GrayUnifyQueryDataSources` 命中 |
| 3 | 接入数据平台（`IS_ACCESS_BK_DATA`）+ cmdb-level 查询 + 表在 `BKDATA_CMDB_LEVEL_TABLES` 白名单 | query.py:383-391 | 仅 `BkdataTimeSeriesDataSource` 命中 |

> 其余情形（多指标、有表达式、有 functions、命中 `AggMethods/CpAggMethods`、无 table 或负 biz_id）一律走 unify-query 分支。

### 1.1 恒走本分支的数据源（id 不在两个列表）

数据源 id = `(data_source_label, data_type_label)`，定义见 `constants/data_source.py:228-241`。

| 数据源类 | id | 备注 |
|----------|-----|------|
| `PrometheusTimeSeriesDataSource` | `("prometheus", "time_series")` | data_source/__init__.py:794 |
| `BkApmTraceTimeSeriesDataSource` | `(BK_APM, TIME_SERIES)` | data_source/__init__.py:2639 |
| `BkMonitorEventDataSource` | `(BK_MONITOR_COLLECTOR, EVENT)` | data_source/__init__.py:2759 |
| `BkFtaEventDataSource` | `(BK_FTA, EVENT)` | data_source/__init__.py:2768 |
| `BkFtaAlertDataSource` | `(BK_FTA, ALERT)` | data_source/__init__.py:2935 |
| `BkMonitorAlertDataSource` | `(BK_MONITOR_COLLECTOR, ALERT)` | data_source/__init__.py:2940 |
...
| 数据源类 | id | 灰度判定 |
|----------|-----|----------|
| `BkdataTimeSeriesDataSource` | `(BK_DATA, TIME_SERIES)` | `BkdataTimeSeriesDataSource.switch_unify_query`（data_source/__init__.py:1368），web 角色恒 True；多指标/有 functions 走 unify；按 `BKDATA_USE_UNIFY_QUERY_GRAY_BIZ_LIST` 灰度 |
| `BkMonitorLogDataSource` | `(BK_MONITOR_COLLECTOR, LOG)` | 继承 `BaseBkMonitorLogDataSource.switch_unify_query`（data_source/__init__.py:2128），按业务白名单/聚类表/全量灰度判定 |
...
| `CustomEventDataSource` | `(CUSTOM, EVENT)` | 继承 `BkMonitorLogDataSource`，内置事件表（`system_event`/`k8s_event`/`cicd_event`）强制 unify（data_source/__init__.py:2692） |
...
`_query_data_using_datasource` 的参数由 `_query_data_internal` 透传（query.py:740-748）：

```python
data, series_stat = self._query_data_using_datasource(
    start_time=start_time,      # 已 process_time_range（query.py:712），缺省回退近 1h
    end_time=end_time,
...
- 返回 `(all_data, all_series_stat)` 二元组；多数据源时相同 key 的 stat 会被后者覆盖（query.py:603 注释说明当前业务通常单数据源）。

### 2.3 后置处理（`process_data_by_datasource`）

`_query_data_internal` 返回前对 `data` 调用 `process_data_by_datasource`（query.py:285）：
...
### 3.1 基类 `TimeSeriesDataSource.query_data`（data_source/__init__.py:1180）
...
### 3.2 `PrometheusTimeSeriesDataSource.query_data`（data_source/__init__.py:863）
...
### 3.3 `BkApmTraceTimeSeriesDataSource` / `BkApmTraceDataSource.query_data`（data_source/__init__.py:2617）
...
`_process_time_range`（data_source/__init__.py:2607）：当 `time_field != "time"` 时把起止时间 ×1000 转毫秒（APM trace 用微/毫秒级时间字段）。

### 3.4 `BkFtaEventDataSource` / `BkMonitorAlertDataSource.query_data`（data_source/__init__.py:2843）
...
`BkFtaAlertDataSource` / `BkMonitorAlertDataSource` 仅覆盖 `data_source_label/data_type_label`，复用上述实现（data_source/__init__.py:2935、2940）。

### 3.5 灰度关闭时的日志/计算平台数据源

- **`BkMonitorLogDataSource.query_data`**（data_source/__init__.py:1948）：按 `topo_nodes` 逐拓扑节点调 `_get_queryset`（bklog DSL），再做 `distinct` 计算与维度前缀清理。
- **`BkdataTimeSeriesDataSource`**（灰度关闭 + cmdb-level）：`_get_queryset`（data_source/__init__.py:1316）将 `_cmdb_level` 表替换为 bk_data 结果表，转调 `BkdataTimeSeriesDataSource._get_queryset` 走 **bk_sql** 查询；指标字段加反引号转义（data_source/__init__.py:1444）。

## 4. 最终后端目标：从 `DataQueryHandler` 到真实存储

所有 `_get_queryset` 最终构造 `DataQueryHandler(cls.data_source_label, cls.data_type_label)`（data_source/__init__.py:649），按 `(data_source_label, data_type_label)` 路由到对应存储后端的查询执行器，链式拼装后执行：
...
`q.raw_data` / `q.original_data` 即原生存储返回。但 `DataQueryHandler` **不是最终落点**——它只是按 id 选择 handler 子类（handler/__init__.py:20），真正执行在 `DataQuery` 基类（models/query.py:210）的 `raw_data`/`original_data` → `compiler.execute_sql()` → `connection.execute(...)`。

### 4.1 handler 路由（handler/__init__.py:20）

下表**仅列出能够实际进入本分支**的 id（即在 `UnifyQueryDataSources` 列表之外，或属于 `GrayUnifyQueryDataSources` 且灰度关闭，或 `BK_DATA` cmdb-level 路由）。

> ⚠️ 重要：`(BK_MONITOR_COLLECTOR, TIME_SERIES)` 与 `(CUSTOM, TIME_SERIES)` **不在本表**——二者在 `UnifyQueryDataSources` 列表内（constants/data_source.py:229、230），`use_unify_query` 对其恒返回 `True`（query.py:347 不命中、后续条件均不返回 `False`），**永远走 unify-query 分支**，不会进入 `_query_data_using_datasource`。`(BK_DATA, LOG)` 虽在 `DATA_SOURCE` 字典中，但无对应 DataSource 类，亦不进入本分支。
...
>
> 映射源：`models/sql/query.py:19` 的 `DATA_SOURCE` 字典（`query` 字段即 `query_func`，`backends` 字段即 backend 模块）；handler 类名来自 `handler/__init__.py:20` 的 `DataQueryHandler.__new__` 路由。
...
| **直连 ES**（不经过 api.*） | `BK_FTA/EVENT`、`BK_FTA/ALERT`、`BK_MONITOR_COLLECTOR/EVENT`、`BK_MONITOR_COLLECTOR/ALERT` | `EventDocument.search(...).update_from_dict(params).execute()`（backends/fta_event/connection.py:33） | **ES**（通过 ES Python 客户端，非 HTTP 网关） |
...
要点：
- **外部 API 路径**：`connection.execute` 把编译好的查询体作为参数传给 `api.<service>.<method>`（core.drf_resource 内部服务网关，本质是 HTTP/RPC 调兄弟服务）。例如 ES backend 的 `execute`（backends/elastic_search/connection.py:43）调 `self.query_func(table_id=rt_id, query_body=params, ...)`，`query_func = api.metadata.get_es_data`。
- **直连 ES 路径**：`fta_event` backend 的 `query_func` 为 `None`（DATA_SOURCE 中 `ALERT/EVENT` 的 `query` 为 `None`），`execute` 直接 `EventDocument.search(all_indices=True).update_from_dict(params).execute()` 命中 ES（带 `es_query` span）。
...
- **elastic_search**（backends/elastic_search/connection.py:43）：`execute(rt_id, params)` → `query_func(table_id=rt_id, query_body=params, use_full_index_names=..., bk_tenant_id=...)`，span=`es_query`。
- **time_series**（backends/time_series/connection.py:48）：`execute(sql, params)` → 拼出 **BKSQL 字符串**，`query_func(sql=sql_str, prefer_storage="", bk_tenant_id=...)`，span=`bksql`。
- **log_search**（backends/log_search）：同上 ES 形态，调 `api.log_search.es_query_search`。
- **fta_event**（backends/fta_event/connection.py:29）：`execute(rt_id, params)` → `EventDocument.search(all_indices=True).update_from_dict(params).execute().to_dict()`，**无 api.* 调用**。

## 5. 最终参数示例

### 5.1 ES 路径示例（`BkMonitorLogDataSource`，id=`BK_MONITOR_COLLECTOR/LOG`）

`_get_queryset` 入参（来自 `BkMonitorLogDataSource.query_data`，data_source/__init__.py:1948）：
...
经 ES `SQLCompiler.as_sql`（backends/elastic_search/compiler.py:103）编译后，`connection.execute(table_id, dsl)` 实际下发的 **ES DSL**（`api.metadata.get_es_data(table_id=..., query_body=dsl)`）：
...
> 该数据源在 `use_unify_query` 中因 cmdb-level 表命中白名单而返回 `False`（query.py:383-391），进入本分支；其 `_get_queryset`（data_source/__init__.py:1316）将 `_cmdb_level` 表替换为 bk_data 结果表，转走 **bk_sql** 查询。

`_get_queryset` 入参（来自 `BkdataTimeSeriesDataSource.query_data` 继承的基类 `TimeSeriesDataSource.query_data`，data_source/__init__.py:1180；`BkdataTimeSeriesDataSource` 覆写 `_get_queryset` 于 1316/1444）：

```python
self._get_queryset(
    bk_tenant_id="tencent",
    metrics=[{"field": "cpu_usage", "method": "AVG", "alias": "cpu_usage"}],
    table="2_system_cpu_detail",          # 原始 _cmdb_level 表，已由 1316 覆写替换为 bk_data 结果表
...
经 `BkdataTimeSeriesDataSource._get_queryset`（1444）给指标字段加反引号、time_series `SQLCompiler` 编译后，`connection.execute(sql, params)` 拼出 **BKSQL 字符串** 并调 `api.bkdata.query_data(sql=..., bk_tenant_id="tencent")`：
...
> 说明：time_series backend 的 `execute`（backends/time_series/connection.py:48）把参数转义后 `%` 拼进 SQL 模板，span=`bksql`；最终由计算平台执行 BKSQL。指标字段在 `BkdataTimeSeriesDataSource._get_queryset`（data_source/__init__.py:1444）中加反引号转义；cmdb-level 表替换逻辑见 1316。
...
`_get_queryset` 入参（来自 `BkFtaEventDataSource.query_data`，data_source/__init__.py:2843，`table` 被替换为时间区间键）：
...
| 内容 | 位置 |
|------|------|
| 分支判定 `use_unify_query` | query.py:342 |
| 本分支入口 `_query_data_using_datasource` | query.py:588 |
| 调用方 `_query_data_internal` else 分支 | query.py:736 |
| 数据源 id 列表 `UnifyQueryDataSources` / `GrayUnifyQueryDataSources` | constants/data_source.py:228 / 234 |
| 基类时序查询 `TimeSeriesDataSource.query_data` | data_source/__init__.py:1180 |
| Prometheus 查询 | data_source/__init__.py:863 |
| APM trace 查询 | data_source/__init__.py:2617 |
| 事件/告警查询 | data_source/__init__.py:2843 |
| 日志数据源查询（灰度关闭） | data_source/__init__.py:1948 |
| 计算平台 cmdb-level 路由 | data_source/__init__.py:1316 |
| 原生查询构造 `DataQueryHandler` | data_source/__init__.py:649 |
| 结果后置 `process_data_by_datasource` | query.py:285 |