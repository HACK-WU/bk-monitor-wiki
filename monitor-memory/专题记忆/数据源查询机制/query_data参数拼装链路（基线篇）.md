...
relation: query_data参数拼装链路（基线篇）
keywords: [to_unify_query_config, get_unify_query_params]
exportedAt: "2026-07-14T03:31:11.945Z"
---
# UnifyQuery.query_data 参数拼装链路

> 调研目标：追踪 `data_source_class(bk_biz_id, interval, metrics, table, group_by)` 的入参如何最终拼成 `query_data` 的 HTTP 请求参数。
> 定位：本文是「UnifyQuery 全查询链路调研」的**基线篇**。其余 `query_data_with_stat` / `query_reference` / `query_log` / `query_dimensions` 均在此基础上扩展或分叉，对照阅读差异最清晰。
> 关联：完整 `to_unify_query_config` 细节见 wiki「统一查询参数拼接链路(data_source_class→query_list→params)」。本文聚焦 `query_data` 这一条链路。
...
- **聚合映射**（`method` 非实时时，见 `to_unify_query_config`:1094）：
...
### 2. UnifyQuery.get_unify_query_params（query.py:415）
...
### 3. UnifyQuery._query_unify_query（query.py:472）
...
### 4. API：QueryDataResource（api/unify_query/default.py:145）
