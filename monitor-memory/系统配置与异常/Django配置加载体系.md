...
#### 关键入口
- `config/tools/environment.py` — 环境/角色/platform 变量初始化
- `config/default.py` — 默认配置（1879 行，含 INSTALLED_APPS、MIDDLEWARE、DATABASES、CACHES、加密、APIGW 等）
- `config/dev.py` — `RUN_MODE="DEVELOP"`，本地 MySQL(Root/空密码)、Redis、DEBUG=True
- `config/role/web.py` — web 角色加载（REST Framework、Swagger、前端中间件、全 INSTALLED_APPS）
- `config/role/api.py` — API 角色加载（精简 INSTALLED_APPS、Token 认证、JWT 中间件、独立日志）
- `config/role/worker.py` — Worker 角色加载（Celery、告警后端配置、无 Web 中间件）
