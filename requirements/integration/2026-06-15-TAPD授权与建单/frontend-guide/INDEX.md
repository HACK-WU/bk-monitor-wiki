...
|--------|------|------------|---------|----------|
| `bound` | 该项目已关联监控业务，可直接建单 | 已关联 | 绿色 | 「去关联」（进入建单流程） |
| `importable` | TAPD 已授权蓝鲸监控，但尚未关联当前业务 | 可关联 | 蓝色 | 「一键关联」（先完成绑定，再走建单流程）⚠️ **当前缺少绑定接口，行为待定，见 [../TODO-待办事项.md](../TODO-待办事项.md)** |
| `stale` | TAPD 侧已解绑蓝鲸监控应用，关联已失效 | 授权失效 | 橙色/黄色 | 「重新授权」（跳转应用安装页重新安装） |
| `unbound` | 该项目在 TAPD 上未授权蓝鲸监控 | 未授权 | 灰色 | 「去授权」（跳转应用安装页首次授权） |
