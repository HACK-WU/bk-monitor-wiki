# -*- coding: utf-8 -*-
"""Check BK-Monitor wiki markdown files against the format spec (R1-R6).

The wiki format requirements are defined in
`bk-monitor-wiki/skills/code-to-wiki/SKILL.md` (Wiki 格式规范 / 格式自检规则 R1-R6):
...
`wiki_incremental.format_validation` (R1/R2/R3/R4/R5 auto-fix; R6 行号需
人工补全) when the package is importable (run with
`PYTHONPATH=bk-monitor-wiki/scripts`).
"""
...
def _check_r3(content: str) -> list[Violation]:
    ...
    for title, start, _end, block in _section_blocks(content):
        ...
        if NO_SOURCE_NOTE_RE.search(block):
            continue
        if not re.search(r"^\*{0,2}章节来源\*{0,2}\s*$", block, re.MULTILINE):
...
def _try_fix(path: Path, content: str) -> tuple[bool, str]:
    try:
        ...
        return False, "（--fix 需要 wiki_incremental 包，请用 PYTHONPATH=bk-monitor-wiki/scripts 运行）"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check BK-Monitor wiki format (R1-R6).")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--wiki-dir", help="待检查的 wiki 目录（递归扫描 *.md）")
    group.add_argument("--file", help="待检查的单个 wiki 文件")
    parser.add_argument("--fix", action="store_true", help="自动修复可机械修复的问题（需 wiki_incremental 包）")
