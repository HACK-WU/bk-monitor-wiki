...
- [告警构建 builder/processor.py](file://bkmonitor/alarm_backends/service/alert/builder/processor.py)
...
## 目录
1. [一、直接调用方矩阵（变更符号 → 消费方）](#一直接调用方矩阵变更符号--消费方)
2. [二、契约与兼容性](#二契约与兼容性)
3. [三、数据流影响](#三数据流影响)
4. [四、未覆盖区域与待确认](#四未覆盖区域与待确认)
5. [五、影响面总评](#五影响面总评)
...
**看图**：三块分别是"告警怎么进来的""状态检查怎么绕开它""动作怎么被拦下"。注意中间和右边两块几乎全是虚线——那些都是"排除掉"的意思，也就是本次改动让这些环节主动跳过托管告警。右边整块没有一条实线，说明这类告警不会触发任何下游动作。

图表来源：综合 [01-逐处解读.md](./01-逐处解读.md) 组 3 / 组 4 与本节调用方矩阵；代码位置见 [shield_tasks.py](file://bkmonitor/alarm_backends/service/alert/manager/shield_tasks.py#L22-L128)、[window.py](file://bkmonitor/alarm_backends/service/converge/shield/window.py#L55-L75)
