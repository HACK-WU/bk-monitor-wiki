...
## 目录
1. [零、A-1 直觉层（零术语开场）](#零a-1-直觉层零术语开场)
2. [一、开场：这段代码为什么存在](#一开场这段代码为什么存在)
3. [二、对外职责边界](#二对外职责边界)
4. [三、公开能力清单（入口）](#三公开能力清单入口)
5. [四、关键抽象](#四关键抽象)
6. [五、两个轮子：定时轮转 vs 按需切分](#五两个轮子定时轮转-vs-按需切分)
7. [六、本次讲解的核心问题清单](#六本次讲解的核心问题清单)
8. [六点五、讲解路线](#六点五讲解路线)
9. [七、知识范围标注说明](#七知识范围标注说明)
10. [八、外部文档索引](#八外部文档索引)
...
[通用] **把无上限增长的记录，按天切成一份份可以整份丢掉的切片，并保证任何时刻都能只翻需要的那几份。**

### 处境对照（带量化锚点）

[专用] 一个中等规模的告警域环境，每天新增数十万条告警记录，且**永不停歇**：

| | 不这么做（全堆在一起） | 这么做（按天切片） |
|---|---|---|
| **清理 30 天前数据** | 逐条扫描比对再删，删完还留下大量空洞需要整理 | 整份删掉，一次操作完成 |
| **查"最近 7 天"** | 要扫遍从第一天起的全部数据 | 只扫 7 份切片 |
| **查"当前还活着"** | 同上，要扫全量 | 只看最新那一份 |
| **单份体积** | 无限增长，出问题时恢复耗时不可控 | 单份有上限（默认 50 GB 再分裂） |

> [通用] 差别不在"能不能查到"，而在**成本随总量增长还是随时间窗增长**：前者会随系统运行年限不断劣化，后者基本恒定。

### 直觉主图

![总账与按天装订的册子](./assets/intuition-ledger-books.svg)

**看图**：上面是一本越写越厚的总账——想找最近七天的记录要从头翻到尾，想扔掉一个月前的记录只能一条条挑出来撕；下面按天分成一本本薄册子——查最近几天只翻这几本，过期了整册丢掉，而"今天还活着的"被搬到最新的那一册里。

**图表来源**（本图为跨文件抽象，不对应单一代码区间；其表达的机制散落在下列位置）
- [ilm.py](file://bkmonitor/bkmonitor/utils/elasticsearch/ilm.py#L29-L62)（`ILM` 构造参数——切片与保留策略的装配入口）
- [ilm.py](file://bkmonitor/bkmonitor/utils/elasticsearch/ilm.py#L257-L370)（`update_index` ——"按天分册"与"换一册"的判定）
- [documents/base.py](file://bkmonitor/bkmonitor/documents/base.py#L78-L89)（`_get_index` ——按文档自身时间决定落哪一册）

### 特性卡

| 特性 | 换来什么 | 代价是什么 |
|---|---|---|
| [通用] 按时间切片 | 清理变整份删除、查询只扫时间窗内切片 | 数据的物理位置由写入时刻决定，跨切片查询要自行拼出切片清单 |
| [专用] 活跃记录每日前移 | "查当前还活着"可以只看最新切片，不必扫全量历史 | 每次建新切片都要重写一遍活跃数据，写入被放大 |
| [专用] 写入按业务时间路由 | 补写历史数据仍落到它本该在的那份切片，时间语义一致 | 写入位置与查询时间窗**天然错位**，长生命周期记录错位严重 |
| [专用] 读切片永久驻留 | 历史查询不受后续切片变化影响，结果稳定 | 切片数量持续增长，别名本身成为需要管理的对象 |
| [专用] 自研而非用现成能力 | 完全掌控搬运条件与保留策略，可贴合告警语义 | 所有边界要自己兜，过期清理与冷热分层至今未真正启用 |
...
[通用] 本模块的四个核心抽象都是 ES 社区通行做法，**但项目里自造了命名**，搜文档时要用业界叫法才搜得到：

| 项目叫法（人话） | 业界标准叫法 | 代码位置 | 代价 / 注意事项 |
|---|---|---|---|
| 物理索引 `bkfta_alert_20260904_0` | Rollover index（ES 官方 rollover 机制生成的索引），`_0/_1` 后缀即 rollover 的序号 | [ilm.py](file://bkmonitor/bkmonitor/utils/elasticsearch/ilm.py#L139-L142) | 一天可能有多个，按名称排序**不等于**按时间排序，别靠名字猜先后 |
| 写别名 `write_20260904_...` | Write alias / write index（ES 官方 `is_write_index` 机制） | [documents/base.py](file://bkmonitor/bkmonitor/documents/base.py#L70-L76) | **本项目特有变体**：业界通常是一个固定的写别名，本项目按日期生成多个——换来"补写历史数据仍落回原日期"，代价是别名数量随天数增长 |
| 读别名 `..._20260904_read` | Read alias（常规别名，一个索引可挂多个） | [documents/base.py](file://bkmonitor/bkmonitor/documents/base.py#L70-L76) | 一个物理索引可挂**多个日期**的读别名——这正是"查 7 天查不到、查 30 天能查到"的根源 |
| 定时轮转 `update_index` | Index rollover（ES 官方 API 同名）；本项目为自实现而非调用官方 API | [ilm.py](file://bkmonitor/bkmonitor/utils/elasticsearch/ilm.py#L257-L370) | 自实现换来"轮转 + 搬运 + mapping 变更"一次做完；代价是要自己保证幂等，官方 API 的那些护栏没有 |
| 大小分裂 `slice` | 业界通常靠 ILM 的 `max_primary_shard_size` 自动 rollover，本项目自实现 | [ilm.py](file://bkmonitor/bkmonitor/utils/elasticsearch/ilm.py#L257-L370) | 阈值 50 GB 是配置项，改它只影响新索引 |
| reindex 搬运 | Reindex API（ES 官方同名）；**"每天把活跃文档搬到当天索引"这套组合是本项目特有** | [ilm.py](file://bkmonitor/bkmonitor/utils/elasticsearch/ilm.py#L244-L255) | 换来"查当前存活只需扫最新索引"；代价是写入被放大（活跃数据每天重写一遍），且搬运期会出现双份文档 |
...
| 顺序 | 讲什么 | 对应产物 |
|---|---|---|
| 1 | 先建立直觉：它把什么变成什么、为什么必须切开 | [00-能力大纲.md](./00-能力大纲.md) |
| 2 | 再认全零件：四个抽象、两个轮子、ilm 与 documents 的分工 | [01-代码wiki.md](./01-代码wiki.md) |
| 3 | 校验：逐条回读代码确认结论 | [02-正确性核对.md](./02-正确性核对.md) |
| 4 | 走一遍真实执行：一轮 update_index 做了哪些判断 | [03-功能推演.md](./03-功能推演.md) |
| 5 | 定位：四类典型症状怎么用它解释，什么场景不适用 | [04-应用场景.md](./04-应用场景.md) |
| 6 | 数据怎么流：一条告警从触发到恢复在索引间怎么走 | [05-数据流.md](./05-数据流.md) |
| 7 | 汇总成一篇可独立阅读的材料 | [06-最终文档.md](./06-最终文档.md) |

> 若只想解决手头问题：先读第零节直觉层建立心智模型，再直接跳 `04-应用场景.md` 对号入座。
...
依据 Phase 1.5 的规模判断——本材料虽标注了 `[通用]` 知识点（索引生命周期、别名路由范式、最终一致性），但通篇**未引用任何外部权威链接**：所有结论均取自本项目源码，通用部分是对既有实现的概念归纳，而非需要对照官方规范验证的版本敏感内容。查阅次数为 0，不满足"≥2 次查阅"的建造门槛。

| 项 | 结论 |
|---|---|
| 涉及的官方站 | Elasticsearch 官方（索引生命周期 / 别名） |
| 是否建索引 | 否 —— 无引用需求，避免为形式合规产出空索引 |
| 影响 | `[通用]` 部分为本项目代码可验证的概念归纳，**不含需外部核实的版本敏感结论** |

> 后续若需补充 ES 原生 ILM 与本自研实现的对比（用于论证"为何自研"），应回到 Phase 1.5 执行 `web-index` 建造模式，并在本表登记。
