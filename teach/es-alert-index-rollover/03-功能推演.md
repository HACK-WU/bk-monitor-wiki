...
## 目录
1. [一、一次轮转周期的完整执行路径](#一一次轮转周期的完整执行路径)
2. [二、reindex 搬运：真实行为推演](#二reindex-搬运真实行为推演)
3. [三、边界与异常路径](#三边界与异常路径)
4. [四、与其它模块的交互点](#四与其它模块的交互点)
5. [五、设计意图推断](#五设计意图推断)
6. [六、性能与复杂度](#六性能与复杂度)
7. [七、隐式行为与副作用清单](#七隐式行为与副作用清单)
...
**看图**：一次轮转的四个参与者，从上到下按时间推进。注意步骤是编号的，编号顺序就是实际执行顺序，中间任何一步返回无需变更就直接结束。
**图表来源**
...
**看图**：这是一个两道关的筛选——先问状态、再问时间，两关都过才搬。两关里任何一关选「否」的结果都一样：留在旧索引不动。所以最终真正被搬走的只是一小部分。
**图表来源**
...
**看图**：中间是核心机制，往外分出三条线——写入、查询、清理各用一套规则。这三条线互不干扰，但都依赖同一套索引命名，这也是为什么改命名规则会同时影响三处。
**图表来源**
