...
📊 配套示意图：[告警文档在索引间的流转](./assets/alert-doc-dataflow.svg)

---

## 目录
1. [一、数据生命周期总览](#一数据生命周期总览)
2. [二、写入流：从告警到物理索引](#二写入流从告警到物理索引)
3. [三、搬运流：reindex 的完整时序](#三搬运流reindex-的完整时序)
4. [四、查询流：时间窗 → 索引列表 → 结果](#四查询流时间窗--索引列表--结果)
5. [五、清理流（当前未启用）](#五清理流当前未启用)
6. [六、异步流与调度全景](#六异步流与调度全景)
7. [七、数据丢失风险清单](#七数据丢失风险清单)
8. [八、待确认清单（汇总）](#八待确认清单汇总)
...
**看图**：这是一条告警文档的一生。从写入开始，大部分会走到稳定期然后终老——只有少数满足条件的会经过搬运和过渡期。注意过渡期是个临时状态，最终会回到稳定。
**图表来源**
...
**看图**：从事件进来到变成文档，重点是那两个时间字段的产生时机——一个是现在，一个是事件发生的时间，两者不是一回事。后面所有的路由判断都基于后者。
**图表来源**
...
**看图**：搬运的完整时序，注意中间的等待环节——不是搬完就完事，还要确认新索引里确实有了，才会去删旧的。这个顺序保证了任何时刻数据都不会丢。
**图表来源**
...
**看图**：查询进来先看类型，三种类型走三条完全不同的路。最后那条是兜底——实在推断不出时间就扫全部索引，代价最大，这是性能问题的常见来源。
**图表来源**
...
**看图**：清理流程的关键在那个配置开关——别名日期解析不出来时，默认是当作没过期、保留，也就是宁可多留也不误删。这是这条链路上的安全设计。
**图表来源**
