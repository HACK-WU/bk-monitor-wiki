...
| 特性 | 换来什么 | 代价是什么 |
|---|---|---|
| [通用] 按时间切片 | 清理变整份删除、查询只扫时间窗内切片 | 物理位置由写入时刻决定，跨切片查询要自行拼清单 |
| [专用] 活跃记录每日前移 | "查当前还活着"只看最新切片 | 每次建新切片都重写一遍活跃数据，写入被放大 |
| [专用] 写入按业务时间路由 | 补写历史数据仍落回它本该在的那份切片 | 写入位置与查询时间窗**天然错位** |
| [专用] 读切片永久驻留 | 历史查询结果稳定 | 切片数量持续增长，别名本身成为管理对象 |
| [专用] 自研而非用现成能力 | 完全掌控搬运条件与保留策略 | 所有边界自己兜，清理与冷热分层至今未启用 |
...
> 📊 **配套示意图**（SVG，可直接在本页渲染）：
> - [组件结构图](./assets/rollover-architecture.svg) —— 编排层 / 存储层 / 消费层三层结构与别名路由
> - [数据流转图](./assets/alert-doc-dataflow.svg) —— 告警文档从接入到查询的完整流转与落点收敛
...
**看图**：横轴是时间，从左到右是三个按天排列的物理索引（真实存放数据的地方），每个物理索引下方挂着一条自己的读别名，随时间累积而永久驻留；最右侧最新那个索引上方另有一条写别名，全系统只有这一条，永远跟着"最新"走。差别在于：读别名是"一个索引一条、只增不减"，写别名是"只有一条、不停搬家"。
...
**看图**：主干是三道串行判定——先看最新索引日期有没有超前（超前就删掉重取），再看体积有没有超 50GB，最后看 mapping 是否一致；只有"体积超限"或"mapping 结构性冲突"这两条才走到右侧的新建分支，其余两条（完全一致、仅新增字段）直接在左侧结束，一次索引都不建。右下角三个分支决定新索引的序号怎么取。
...
**看图**：从上方旧索引的文档出发，连续过两道闸——先是状态必须是 ABNORMAL（已恢复的不搬），再是 create_time 不早于新索引日期（长期存活的旧告警不搬）；两道都过才真正搬运，搬完立刻删旧副本，而删除前的那一小段重叠期里新旧两份并存，靠按更新时间排序去重来兜住。
...
**看图**：中心是 ILM，向下游分出三条独立的边——写入侧、查询侧、清理侧，三者互不干扰；清理侧那条最终指向一个打了警告标记的节点，表示调度并未真正注册。左侧用虚线连过来的是元数据域的同名实现，虚线强调的是"名字一样但不是一回事"，不是调用关系。
...
| [assets/intuition-ledger-books.svg](./assets/intuition-ledger-books.svg) | 零术语直觉主图：总账 vs 按天装订的册子 |
| [assets/index-layout-and-aliases.svg](./assets/index-layout-and-aliases.svg) | 索引布局与别名路由示意图 |
| [assets/rollover-architecture.svg](./assets/rollover-architecture.svg) | 组件结构图：编排层 / 存储层 / 消费层与别名路由 |
| [assets/alert-doc-dataflow.svg](./assets/alert-doc-dataflow.svg) | 数据流转图：告警文档在索引间的流转与落点收敛 |
