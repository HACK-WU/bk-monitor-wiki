...
## 目录
1. [一、一轮 `sync_issue_alert_stats` 的完整执行路径](#一一轮-sync_issue_alert_stats-的完整执行路径)
2. [二、正常路径：一条告警被漏关联后如何被补回](#二正常路径一条告警被漏关联后如何被补回)
...
**看图**：从顶上"每 5 分钟"那个起点往下走，先做一次哨兵检查（可能直接跳过），然后进入主循环——每个活跃 Issue 依次过五道关，最后回到"还有下一条"的判断形成一圈。注意右上角两个菱形是提前跳过的出口，不是所有 Issue 都会走完五步。

**图表来源**：[issue_tasks.py](file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L47-L229)
...
- `file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L47-L229`（主流程）
- `file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L231-L384`（backfill 与 7 天窗口）
- `file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L456-L767`（impact_scope）
- `file://bkmonitor/alarm_backends/service/scheduler/tasks/cron.py#L88-L110`（expires 计算与 cluster 分发）
- `file://bkmonitor/bkmonitor/management/commands/backfill_issue_fingerprint.py#L28-L35`（历史修复命令）
