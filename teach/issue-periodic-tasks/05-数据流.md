...
## 目录
1. [一、数据在模块内的生命周期](#一数据在模块内的生命周期)
2. [二、状态流转](#二状态流转)
3. [三、派生字段的更新时序](#三派生字段的更新时序)
4. [四、异步流与队列](#四异步流与队列)
5. [五、数据一致性与丢失风险](#五数据一致性与丢失风险)
6. [六、一处值得注意的时序耦合](#六一处值得注意的时序耦合)
7. [章节来源](#章节来源)
...
**看图**：左中右三栏分别是"从哪读""做什么处理""写回哪"。顺着箭头看，每件事都是一条"读一样 → 处理 → 写一样"的直线。唯一那条虚线是例外——它是另一个任务写的，而且是异步的，不属于周期任务本身。
...
**看图**：起点是"还没归属"的告警，它有两条出路——正常那条由主链路走，漏了的那条由周期任务补。关键在两条路的终点是一样的，这正是"重复执行也不会写坏"的原因。

**图表来源**：[issue_tasks.py](file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L320-L384)
...
- `file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L47-L111`（主循环）
- `file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L157-L229`（派生字段写入）
- `file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L320-L384`（backfill 状态变化）
- `file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L1055-L1082`（CAS 写入与时序说明）
- `file://bkmonitor/alarm_backends/service/scheduler/tasks/cron.py#L88-L110`（队列与 expires）
- `file://bkmonitor/alarm_backends/core/cache/key.py#L1248-L1260`（哨兵 TTL）
