...
- [聚合主链路 issue_processor.py](file://bkmonitor/alarm_backends/service/fta_action/issue_processor.py)
...
> **一句话本质**：主流程是"来一条处理一条"，忙起来难免漏；这两个任务**每隔一阵回头走一圈，把漏掉的补回去、把总数重新数一遍**。
>
...
| 特性 | 换来什么 | 代价是什么 |
|---|---|---|
| 所有写入都设计成可重复执行 | 多集群各跑一遍也不会把数据写坏 | 重复执行纯粹浪费算力，每多一个集群负载翻一倍 |
| 一个任务体内塞 5 项职责 | 一轮扫描共享同一次全量遍历，省 4 次扫描 | 任务体长、职责耦合，任一项抛错影响同轮后续项 |
| 指纹复算复用主链路同一函数 | 补偿进来的告警与正常进来的一定是同一口径 | 主链路一改算法两边同时变，没有缓冲 |
| 单周期内按策略去重 | 把 N×M 的查询量降到 N+M | 去重集合只在本轮有效，跨轮不复用 |
...
**章节来源**：`file://bkmonitor/config/role/worker.py#L236-L244`、`file://bkmonitor/alarm_backends/service/fta_action/tasks/__init__.py#L18-L22`
...
**看图**：上栏是主链路，看那个菱形——抢锁失败就直接走到"静默丢失"，没有任何补救。中栏列出它留下的两个坑，下栏是周期任务分别怎么填。上下两栏一一对应：一个坑对应一个补偿动作。

**图表来源**：[issue_processor.py](file://bkmonitor/alarm_backends/service/fta_action/issue_processor.py#L120-L162)、[issue_tasks.py](file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L157-L229)
...
**看图**：三块分别是两个周期任务和一个异步任务。左边那块五步是串行的，每 5 分钟完整走一遍；右边两块之间没有箭头相连，唯一那条虚线表示"每小时那个任务产出的东西，会被异步任务拿去用"——它们是协作关系，不是调用关系。

**图表来源**：[issue_tasks.py](file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L48-L56)、[issue_tasks.py](file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L1439-L1451)、[issue_tasks.py](file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L1085-L1096)
...
**看图**：三栏分别是"从哪读""做什么""写回哪"，每件事都是一条从左到右的直线。顺着箭头数一遍：五种读、五种处理、四种写——其中两种处理最终写到同一个地方。

**图表来源**：[issue_tasks.py](file://bkmonitor/alarm_backends/service/fta_action/tasks/issue_tasks.py#L47-L1531)
