...
**看图**：从上往下是一次页面查询的完整往返。中间那个 `alt` 分支是唯一的例外——趋势图不建树，直接拿原始数字画；其余所有图都要先经过建树那一步。注意最后一步是"逐个出图再合并"，不是一次查一张图。
...
**看图**：这是一条单向流水线，走到哪一步就把状态改成哪一步，所以任何时刻都能从状态看出卡在哪。两条岔路都通向失败态而不是重试——解析失败和回灌失败都不会自动重来。
