...
**看图**：主干是一条直线——取回原始行、建树、算值、出图。关键在中间的分叉：同一批原始行会走两条路，一条进树再变成图，另一条跳过建树直接画时序。数据形态在"建树"这一步发生质变，前面是行、后面是树。
...
**看图**：上半段是同步的（存文件、建记录），下半段交给后台异步做——真正耗时的解析和回灌都在那条虚线之后。中间那个菱形是"依次试解析器"，试完两种都不行就进失败态，没有第三选择。
...
**看图**：起点只有一个，但有三条出路——两个失败态是死路，不会自动回到起点重来。唯一有分支的是中间那步：解析成功后，回灌还可能失败，且失败前会重试三次。
