...
> **一句话本质**：把"程序运行时每一刻在忙什么"的原始记录，变成一张**哪里最耗时、一眼就能看出来**的图。
>
...
| 特性 | 换来什么 | 代价是什么 |
|---|---|---|
| 先建一棵调用栈树，再出图 | 六种图共用同一份树，新增一种图不用重新解析 | 树建错则所有图一起错 |
| 页面主链路绕开 pprof 中间格式 | 少一次序列化，查询链路更短 | 页面与上传 / 导出走两套路径，改一处要确认两边 |
| 解析器与图表都用注册表 | 新增一种格式 / 图 = 加一个文件 + 一行注册 | 调用关系在代码里看不出来，新人找不到实现 |
| 上传的文件回灌进采集链路 | 手工上传与自动采集共用同一条通路 | 依赖外部采集端可用，它挂了上传就失败 |
...
**看图**：上半条是"看"的链路——取回原始记录、建树、出图；下半条是"传"的链路——上传的文件转成标准格式后回灌进存储。两条线共用同一个存储，但页面主链路只走上面那条。
...
**看图**：从上往下是一次页面查询的完整往返。中间那个分支是唯一例外——趋势图不建树，直接拿原始数字画；其余所有图都要先经过建树。最后是"逐个出图再合并"，不是一次查一张图。
...
注意 `STORE_FAILED` 的措辞：`3 次 HTTP 尝试` 发生在**同一次调用内**，不是任务级重试——失败后不会重新排队。

**看图**：起点只有一个，但有三条出路——两个失败态是死路，不会自动回到起点重来。唯一有分支的是中间那步：解析成功后，回灌还可能失败。
