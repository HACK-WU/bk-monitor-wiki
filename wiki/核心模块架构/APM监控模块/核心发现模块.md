...
- [discover/node.py:40-465](file://bkmonitor/apm/core/discover/node.py#L40-L465)
...
- 合并策略（Node 工具方法）：combine_sources 合并来源列表；merge_other_extra_data_preserving_category 在保留 category 前提下合并额外 extra_data；merge_other_discovery 合并其它发现结果；merge_platform 合并平台信息（platform 字段）。
...
图表来源
- [discover/node.py:107-465](file://bkmonitor/apm/core/discover/node.py#L107-L465)
